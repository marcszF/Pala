...
-- buildIdSet deduplicates entries as it builds the lookup tables.
local useIdSet = rawget(_G, "PalaUseIdSet")
if not useIdSet then
    useIdSet = buildIdSet(useIds)
    _G.PalaUseIdSet = useIdSet
end
local stepIdSet = rawget(_G, "PalaStepIdSet")
if not stepIdSet then
    stepIdSet = buildIdSet(stepIds)
    _G.PalaStepIdSet = stepIdSet
end
